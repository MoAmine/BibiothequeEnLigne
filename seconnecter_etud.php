    ...
    session_start();
       ...
       if (isset($_POST['submit'])) {
            ...
            if($email=="admin@admin.fsj" && $mdp=="123456"){ 
              ...
              $sql_etud="SELECT * FROM etudiants WHERE email='$email' AND mdp='$mdp'";
              $resultat_prof=mysqli_query($bdd,$sql_prof);
              $resultat_etud=mysqli_query($bdd,$sql_etud);
                  ...
                  if($resultat_prof && mysqli_num_rows($resultat_prof)!=0)
                     ...
                     }else if($resultat_etud && mysqli_num_rows($resultat_etud)!=0){
                            $row=mysqli_fetch_assoc($resultat_etud);
                            $_SESSION['id_etudiant']=$row['id_etudiant'];
                            header("Location: etudiants/accueil.php");
                           exit();
